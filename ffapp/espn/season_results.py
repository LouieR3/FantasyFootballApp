...
def add_playoff_results(league):
    try:
        ...
        # Read the LPI data
        lpi_df = pd.read_excel(f"{LEAGUES_DIR}/{fileName}.xlsx", sheet_name=sheet_name, index_col=0)  # Team names as index
        ...
        # Open the workbook
        workbook = openpyxl.load_workbook(f"{LEAGUES_DIR}/{fileName}.xlsx")
        # Check if the sheet already exists
        if "Playoff Results" in workbook.sheetnames:
            # Remove the sheet
            del workbook["Playoff Results"]
        # Save the workbook after removing the sheet
        workbook.save(f"{LEAGUES_DIR}/{fileName}.xlsx")

        # Add playoff_df as a new sheet to the existing Excel file
        with pd.ExcelWriter(f"{LEAGUES_DIR}/{fileName}.xlsx", engine="openpyxl", mode="a") as writer:
