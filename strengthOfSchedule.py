...
# Create an empty list to store game results as dictionaries
game_results_data = []

# Iterate through the teams and their schedules
for team_name in team_names:
    team_schedule = schedules_df.loc[team_name].tolist()
    team_scores = scores_df.loc[team_name].tolist()

    for week, opponent in enumerate(team_schedule):
        # Avoid comparing against an empty schedule
        if not pd.isna(opponent):
            # Extract scores
            team_score = team_scores[week]
            opponent_score = scores_df.loc[opponent][week]

            # Create a dictionary for the game result
            game_result = {
                'HomeTeam': team_name,
                'AwayTeam': opponent,
                'HomeScore': team_score,
                'AwayScore': opponent_score
            }

            # Append the game result dictionary to the list
            game_results_data.append(game_result)

# Create the game_results_df DataFrame by concatenating the list of dictionaries
game_results_df = pd.DataFrame(game_results_data)

# Display the resulting game_results_df
...
def oddsCalculator():
  ...
  for team in position_chances_df.index:
    ...
    # Calculate the sum and explicitly cast it to the appropriate data type (e.g., float)
    total_chance = float(top_finishes.sum())
    
    position_chances_df.at[team, 'Chance of making playoffs'] = total_chance
...
print()
# Get a list of unique team names from game_results_df
game_teams = game_results_df['AwayTeam'].unique()

# Get a list of unique team names from lpi_df
lpi_teams = lpi_df.index.unique()

# Find the missing teams
missing_teams = set(game_teams) - set(lpi_teams)

# Print or handle the missing teams as needed
print(game_teams)
print(lpi_teams)
print("Missing teams:", missing_teams)


# Calculate average LPI of opponents for each team
sos_df = game_results_df.groupby('HomeTeam')['AwayTeam'].apply(lambda x: lpi_df.loc[x].mean()).reset_index()
sos_df.columns = ['Team', 'AverageOpponentLPI']

# Sort teams based on SOS
sos_df = sos_df.sort_values(by='AverageOpponentLPI')
