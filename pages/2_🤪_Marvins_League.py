...
def app():
    ...
    league_id = 1616305229
    espn_s2=CRED["louie_s2"]
    swid=CRED["louie_swid"]
    # Seasons with data on file - no hard-coded list to keep in sync
    year_options = available_years("The Goofy Goobers")
    if not year_options:
        st.error("No season data found for The Goofy Goobers.")
        return
    selected_year = st.selectbox(
        "Select Year", year_options, index=len(year_options) - 1
    )

    league = f"The Goofy Goobers {selected_year}"
