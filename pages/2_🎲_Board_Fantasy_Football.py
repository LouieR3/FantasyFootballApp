...
def app():
    ...
    league_id = 496646254
    espn_s2=CRED["nolan_s2"]
    swid=CRED["nolan_swid"]
    # Seasons with data on file - no hard-coded list to keep in sync
    year_options = available_years("Board Fantasy Football")
    if not year_options:
        st.error("No season data found for Board Fantasy Football.")
        return
    selected_year = st.selectbox(
        "Select Year", year_options, index=len(year_options) - 1
    )

    league = f"Board Fantasy Football {selected_year}"
