...
def app():
    ...
    year_options = available_years("Campers and Skiers and Prahlad")
    if not year_options:
        st.error("No season data found for Campers and Skiers and Prahlad.")
        return
    selected_year = st.selectbox(
        "Select Year", year_options, index=len(year_options) - 1
    )

    league = f"Campers and Skiers and Prahlad {selected_year}"
