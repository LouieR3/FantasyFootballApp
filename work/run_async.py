    ...
    async with session.get(url) as response:
        ...
        if response.status == 200:
            ...
            if parcel_table is not None and owner_table is not None:
                ...
                # print(data_df)
                data_df.to_csv("c:/Users/louie/Desktop/repo/FantasyFootballApp/work/parcel_data.csv", mode="a", header=False, index=False)
                return data_df
            else:
                return None
...
async def main():
    ...
    while True:
        ...
        if response.status_code == 200:
            ...
            break
    csv_df = pd.read_csv("c:/Users/louie/Desktop/repo/FantasyFootballApp/work/parcel_data.csv")
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        #     combined_df.to_csv("parcel_data.csv", mode="a", header=False, index=False)
        #     dfs_to_concat = []
        # except Exception as e:
        #     print("Saving data up to failure")
        #     combined_df = pd.concat(datas, ignore_index=True)
