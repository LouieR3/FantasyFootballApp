...
# # Compare LPI ranks at the start of playoffs with final standings
# final_standings = all_playoff_dfs.groupby('Winner')['LPI 1'].max()
# lpi_rankings = lpi_df.loc[final_standings.index].mean(axis=1).rank()

# correlation = lpi_rankings.corr(final_standings.rank())
# print(f"LPI accuracy correlation with final standings: {correlation:.2f}")
